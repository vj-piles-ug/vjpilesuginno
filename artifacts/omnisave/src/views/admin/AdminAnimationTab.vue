<template>
  <div class="tab-page">
    <div class="tab-header">
      ...
      <p class="tab-sub">Upload animated movies and series</p>
    </div>
    <div class="form-card">
      ...
      <div class="form-grid">
        ...
        <div class="field col-2">
          ...
          <input v-model="form.streamUrl" placeholder="https://example.com/stream/anime.mp4" />
        </div>
        <div class="field col-2">
      ...
      <button class="btn-add" @click="addItem" :disabled="!form.title || !form.streamUrl">Add Animation</button>
      <p v-if="saved" class="save-msg">✓ Added successfully</p>
    </div>
    <div class="items-grid">
      ...
      <div v-if="items.length === 0" class="empty">No animation added yet.</div>
    </div>
  </div>
</template>
...
interface AnimItem { id: number; title: string; type: string; streamUrl: string; posterUrl: string; rating: number; year: number }
let nextId = 1
const items = ref<AnimItem[]>([])
const saved = ref(false)
const form = ref({ title: '', type: 'Movie', streamUrl: '', posterUrl: '', rating: 7.5, year: new Date().getFullYear() })

function addItem() {
  items.value.push({ id: nextId++, ...form.value })
  form.value = { title: '', type: 'Movie', streamUrl: '', posterUrl: '', rating: 7.5, year: new Date().getFullYear() }
  saved.value = true
...
.ic-type { position: absolute; top: 8px; left: 8px; font-size: 0.58rem; font-weight: 800; padding: 2px 7px; border-radius: 4px; background: rgba(124,58,237,0.8); color: #fff; }
