<template>
  <div class="tab-page">
    <div class="tab-header">
      ...
      <p class="tab-sub">Total: {{ adminStore.carouselItems.length }} items</p>
    </div>
    ...
    <div class="form-card">
      ...
      <button class="btn-add" @click="addItem" :disabled="!form.title || !form.imageUrl">Add Item</button>
    </div>

    <!-- Items List -->
    <div class="items-list">
      <div v-for="item in adminStore.carouselItems" :key="item.id" class="carousel-row">
        <div class="ci-thumb">
        ...
        <div class="ci-actions">
          ...
          <button class="ci-btn-delete" @click="deleteItem(item.id)" title="Delete">
      ...
      <div v-if="adminStore.carouselItems.length === 0" class="empty">No carousel items yet.</div>
    </div>
    ...
    <div v-if="editItem" class="modal-overlay" @click.self="editItem = null">
      <div class="modal">
        ...
        <div class="field"><label>Title</label><input v-model="editItem.title" /></div>
        <div class="field"><label>Subtitle</label><input v-model="editItem.subtitle" /></div>
        <div class="field"><label>Banner Image URL</label><input v-model="editItem.imageUrl" /></div>
        <div class="modal-actions">
          <button class="btn-ghost" @click="editItem = null">Cancel</button>
...
import { ref } from 'vue'
import { adminStore, type CarouselItem } from '../../store/admin'

const form = ref({ title: '', subtitle: '', imageUrl: '', linkContent: 'No Link' })
const editItem = ref<CarouselItem | null>(null)

function addItem() {
  adminStore.carouselItems.push({
    id: adminStore.nextCarouselId++,
    title: form.value.title,
    subtitle: form.value.subtitle,
    imageUrl: form.value.imageUrl,
    linkContent: form.value.linkContent,
  })
  form.value = { title: '', subtitle: '', imageUrl: '', linkContent: 'No Link' }
}

function deleteItem(id: number) {
  const i = adminStore.carouselItems.findIndex(c => c.id === id)
  if (i !== -1) adminStore.carouselItems.splice(i, 1)
}
...
function saveEdit() {
  ...
  const i = adminStore.carouselItems.findIndex(c => c.id === editItem.value!.id)
  if (i !== -1) adminStore.carouselItems[i] = { ...editItem.value }
  editItem.value = null
}
...
.ci-title { font-size: 0.88rem; font-weight: 600; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.ci-sub { font-size: 0.72rem; color: rgba(255,255,255,0.4); margin-top: 2px; }
.ci-actions { display: flex; gap: 8px; flex-shrink: 0; }
.ci-btn-edit { padding: 7px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: rgba(255,255,255,0.6); cursor: pointer; display: flex; transition: background 0.15s; }
.ci-btn-edit:hover { background: rgba(255,255,255,0.1); color: #fff; }
.ci-btn-delete { padding: 7px; background: rgba(220,38,38,0.1); border: 1px solid rgba(220,38,38,0.25); border-radius: 8px; color: #f87171; cursor: pointer; display: flex; transition: background 0.15s; }
