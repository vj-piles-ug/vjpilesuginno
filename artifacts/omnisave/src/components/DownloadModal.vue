    ...
    <Transition name="modal">
      <div v-if="movie" class="modal-backdrop" @click.self="$emit('close')">
        <div class="modal-box">
          ...
          <!-- TV SERIES: episode selector + download -->
          <template v-else>
            ...
            <button class="big-dl-btn" :disabled="!selectedEp" @click="$emit('close')">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/>
                <polyline points="7 10 12 15 17 10"/>
                <line x1="12" y1="15" x2="12" y2="3"/>
...
.modal-poster { width: 46px; height: 62px; flex-shrink: 0; border-radius: 8px; display: flex; align-items: center; justify-content: center; }
...
.big-dl-btn:disabled { opacity: 0.45; cursor: not-allowed; filter: none; background: rgba(255,255,255,0.15); color: rgba(255,255,255,0.4); box-shadow: none; }
