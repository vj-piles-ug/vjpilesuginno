...
const posterStyle = computed(() => ({
  background: liquidPalettes[gradientIdx.value % liquidPalettes.length],
}))
...
.poster-gradient { position: absolute; inset: 0; }
...
.shimmer-layer {
  ...
  background-size: 200% 200%; animation: shimmer-move 3.5s ease-in-out infinite;
}
...
.poster-top { position: absolute; top: 6px; left: 6px; right: 6px; display: flex; align-items: center; gap: 3px; flex-wrap: wrap; z-index: 2; }
.poster-badge { font-size: 0.55rem; font-weight: 800; letter-spacing: 0.06em; padding: 2px 6px; border-radius: 3px; text-transform: uppercase; line-height: 1.4; }
...
.poster-dl-btn {
  ...
  color: #021a10; font-size: 0.58rem; font-weight: 800; letter-spacing: 0.1em;
  cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 4px;
  box-shadow: 0 4px 14px rgba(0,255,157,0.22); transition: filter 0.15s;
}
.poster-dl-btn:hover { filter: brightness(1.07); }
</style>
